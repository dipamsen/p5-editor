...
import { resolveLinksJS, resolveLinksCSS, resolveScripts, resolveStyles, resolveFileFromPath, } from "../utils/CodeAnalysis";
import { MEDIA_FILE_REGEX } from "../utils/FileUtils";
import { consoleErrorScript } from "../utils/defaultFiles";
export const renderSketch = action((state, editor) => {
    const htmlFile = state.files.find((file) => file.name?.match(/.*\.html$/))?.content;
    const resolvedFiles = [];
