// From https://github.com/processing/p5.js-web-editor/blob/develop/server/utils/fileUtils.js
...
text/csv,.obj,video/webm,video/ogg,video/quicktime,video/mp4,application/xml` as const;

export const fileExtensions = fileExtensionsArray
  .map((ext) => `.${ext}`)
  .join(",");
export const fileExtensionsAndMimeTypes =
  `${fileExtensions},${mimeTypes}` as const;
...
export const NOT_EXTERNAL_LINK_REGEX = /^(?!(http:\/\/|https:\/\/|blob:))/;
export const EXTERNAL_LINK_REGEX = /^(http:\/\/|https:\/\/|blob:)/;
