import { STRING_REGEX, MEDIA_FILE_QUOTED_REGEX, NOT_EXTERNAL_LINK_REGEX, PLAINTEXT_FILE_REGEX, } from "./FileUtils";
export function resolveLinksJS(content, files) {
...
export function resolveLinksCSS(content, files) {
    const strings = content.match(STRING_REGEX) || [];
    strings.forEach((str) => {
        if (str.match(MEDIA_FILE_QUOTED_REGEX)) {
            // Media File String Found
...
export function resolveScripts(doc, files) {
...
export function resolveStyles(doc, files) {
    ...
    const cssLinksInHTML = Array.from(doc.querySelectorAll('link[rel="stylesheet"]'));
...
export function resolveFileFromPath(path, allFiles) {
    const file = allFiles.find((f) => f.name == path.split("/").pop());
    if (file) {
        if (file.blobUrl && !file.name.match(PLAINTEXT_FILE_REGEX))
            return file;
