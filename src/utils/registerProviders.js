...
export default function registerProviders() {
    ...
    monaco.languages.registerCompletionItemProvider("html", new HTMLClosingTagProvider());
    // JavaScript Snippets
    monaco.languages.registerCompletionItemProvider("javascript", new JSSnippetsProvider());
    ...
    for (const file of files) {
        monaco.languages.typescript.javascriptDefaults.addExtraLib(file.content, file.name);
        monaco.editor.createModel(file.content, "typescript", monaco.Uri.file(file.name));
    }
    ...
    window.typings = {
        add(uri, name) {
            fetch(uri)
                ...
                monaco.languages.typescript.javascriptDefaults.addExtraLib(lib, u.toString());
            });
        },
    };
    // // ESLint Code Action
